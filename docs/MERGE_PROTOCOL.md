...
`RN50` 使用 full recipe 后单模型 best R1 只有 `27.015`。将其以极小权重加入 `ViT-B/16 + ViT-B/32` 融合池后没有超过两专家最优 `49.617`：
...
同时发现 RN50 的 OpenAI `visual.attnpool.positional_embedding` 需要从原始 `50` tokens resize 到当前 `(384,128)` 下的 `49` tokens；旧代码未 resize，导致 RN50 专家训练时该参数随机初始化。代码已补 `resize_attnpool_pos_embed`，但现有 RN50 checkpoint 仍视为低质量探索结果。正式融合池暂不纳入 RN50，除非重训后 R1 或互补性明显改善。

### ViT-L/14 试验记录

`ViT-L/14` 使用 `BATCH_SIZE=16` 完成 full recipe 训练，单模型 best R1 为 `41.589`。加入 `ViT-B/16 + ViT-B/32` 融合池后带来进一步提升：
...
- 当前方法主结果更新为 `GAR-EM prior 0.82,0.08,0.10`，R1 `49.813`。
- 相比 full-module 主专家 `49.064` 提升 `+0.749`。
- 相比两专家 GAR-EM 最优 `49.617` 提升 `+0.196`。
- 增加 ViT-L/14 到 `0.12-0.14` 可继续提升 mAP/mINP，但 R1 回落，说明 R1-oriented 主结果应停在 `0.82,0.08,0.10`。
- ViT-L/14 对 mAP/mINP 有明显帮助，提示大模型专家对整体排序质量有互补价值。
