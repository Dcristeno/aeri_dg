...
- RN50 full recipe 单模型 best R1：`27.015` at epoch 50
- SwanLab 链接：https://swanlab.cn/@Dcristen/CFAN/runs/7sovisvxqyzj089a9bao1
- 训练/融合时发现 RN50 OpenAI 预训练的 `visual.attnpool.positional_embedding` 从原始 `50` tokens 适配到当前 `(384,128)` 时应为 `49` tokens，旧代码未 resize，导致该参数随机初始化；已补 `resize_attnpool_pos_embed`，后续若要重新评估 RN50，建议重训。

| method | R1 | R5 | R10 | RSum | mAP | mINP |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| mean 3 experts | 47.126 | 63.817 | 72.675 | 183.618 | 45.007 | 32.392 |
| fixed `0.92,0.07,0.01` | 49.210 | 67.188 | 75.444 | 191.842 | 47.271 | 34.266 |
| GAR-EM prior `0.92,0.07,0.01` | 49.308 | 66.943 | 75.085 | 191.337 | 47.446 | 34.583 |
| fixed `0.90,0.08,0.02` | 49.357 | 67.318 | 75.427 | 192.102 | 47.390 | 34.406 |
| GAR-EM prior `0.90,0.08,0.02` | 49.308 | 66.732 | 75.020 | 191.060 | 47.485 | 34.706 |
| fixed `0.94,0.055,0.005` | 49.064 | 67.285 | 75.411 | 191.760 | 47.144 | 34.074 |
| GAR-EM prior `0.94,0.055,0.005` | 49.422 | 67.009 | 75.167 | 191.597 | 47.417 | 34.486 |

结论：当前 RN50 专家过弱，加入三专家融合后没有超过两专家最优 `49.617`。在修复 RN50 attention-pool 位置编码适配并重训之前，不把 RN50 放入正式融合池。

### ViT-L/14 三专家尝试

- ViT-L/14 full recipe 使用 `BATCH_SIZE=16` 训练完成
- SwanLab 链接：https://swanlab.cn/@Dcristen/CFAN/runs/v96cycgg32v6zgewbco15
- ViT-L/14 单模型 best R1：`41.589` at epoch 58
- 融合池：`ViT-B/16 + ViT-B/32 + ViT-L/14`

| method | R1 | R5 | R10 | RSum | mAP | mINP |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| mean 3 experts | 49.113 | 65.527 | 73.929 | 188.569 | 48.131 | 36.539 |
| fixed `0.90,0.08,0.02` | 49.178 | 67.399 | 75.525 | 192.102 | 47.438 | 34.502 |
| GAR-EM prior `0.90,0.08,0.02` | 49.520 | 67.009 | 75.199 | 191.728 | 47.798 | 35.000 |
| fixed `0.87,0.08,0.05` | 49.373 | 67.562 | 75.558 | 192.493 | 47.736 | 34.893 |
| GAR-EM prior `0.87,0.08,0.05` | 49.487 | 66.911 | 75.134 | 191.532 | 48.035 | 35.367 |
| fixed `0.82,0.08,0.10` | 49.666 | 67.660 | 75.607 | 192.933 | 48.118 | 35.482 |
| GAR-EM prior `0.82,0.08,0.10` | 49.813 | 66.650 | 74.955 | 191.418 | 48.307 | 35.797 |
| fixed `0.80,0.08,0.12` | 49.601 | 67.530 | 75.558 | 192.688 | 48.243 | 35.642 |
| GAR-EM prior `0.80,0.08,0.12` | 49.764 | 66.781 | 74.776 | 191.321 | 48.363 | 35.940 |
| fixed `0.78,0.08,0.14` | 49.699 | 67.546 | 75.704 | 192.949 | 48.362 | 35.822 |
| GAR-EM prior `0.78,0.08,0.14` | 49.731 | 66.699 | 74.760 | 191.190 | 48.424 | 36.061 |
| fixed `0.84,0.06,0.10` | 49.601 | 67.660 | 75.655 | 192.916 | 48.022 | 35.320 |
| GAR-EM prior `0.84,0.06,0.10` | 49.780 | 66.862 | 75.037 | 191.679 | 48.286 | 35.738 |

结论：

- ViT-L/14 虽然单模型 R1 只有 `41.589`，但比 ViT-B/32 和 RN50 更有融合价值。
- 三专家 `GAR-EM prior 0.82,0.08,0.10` 达到当前最高 R1 `49.813`，相比 full-module 主专家 `49.064` 提升 `+0.749`，相比两专家最优 `49.617` 提升 `+0.196`。
- 继续增加 ViT-L/14 权重到 `0.12-0.14` 会提升 mAP/mINP，但 R1 从 `49.813` 回落到 `49.764/49.731`，说明当前 R1 最优点在 ViT-L/14 约 `0.10`。
- ViT-L/14 明显提升 mAP/mINP，说明它可能补充了中后段排序质量；R5/R10/RSum 仍需和 fixed 权重对照讨论。
- 当前融合主结果建议更新为 `ViT-B/16 + ViT-B/32 + ViT-L/14` 的 `GAR-EM prior 0.82,0.08,0.10`。
