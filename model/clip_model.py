...
class CLIP(nn.Module):
    ...
    def forward(self, image, ground_image,text):
        if text.size(0)==2*image.size(0) :
            image_features = self.encode_image(image,modal='visual')
            if ground_image is not None:
                ground_image_features = self.encode_image(ground_image,modal='visual')
            else:
                ground_image_features = None
            text_features = self.encode_text(text,modal='text')
        else:
            image_features = self.encode_image(image)
            if ground_image is not None:
                ground_image_features = self.encode_image(ground_image)
            else:
                ground_image_features = None
            text_features = self.encode_text(text)

        return image_features, ground_image_features, text_features
