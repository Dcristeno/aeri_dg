...
def get_args():
    ...
    parser.add_argument("--hard_negative_loss_weight", type=float, default=1.0, help="overall weight for the PID-aware hard-negative margin loss")
    parser.add_argument("--hard_negative_margin", type=float, default=0.2, help="cosine margin between positives and hard negative cluster")
    parser.add_argument("--hard_negative_topk", type=int, default=1, help="number of hardest negatives averaged as the negative cluster")
    parser.add_argument("--hard_negative_positive_reduce", type=str, default="max", help="positive reducer for same-PID matches: max, mean, or logsumexp")
