...
def get_args():
    ...
    parser.add_argument("--fta_loss_weight", type=float, default=1.0, help="weight for the original FTA fuzzy alignment loss")
    parser.add_argument("--fta_hard_weight", type=float, default=0.0, help="weight for the FTA hard-negative ranking auxiliary loss")
    parser.add_argument("--fta_hard_margin", type=float, default=0.05, help="margin for the FTA hard-negative ranking auxiliary loss")
    parser.add_argument("--moe_num_experts", type=int, default=4, help="number of experts in the feature-level MoE adapter")
    parser.add_argument("--moe_hidden_dim", type=int, default=256, help="hidden dimension of each feature-level MoE expert")
    parser.add_argument("--moe_residual_scale", type=float, default=0.2, help="residual scale for feature-level MoE adapters")
    parser.add_argument("--moe_dropout", type=float, default=0.0, help="dropout inside each feature-level MoE expert")
    parser.add_argument("--moe_balance_weight", type=float, default=0.01, help="weight for MoE expert load-balancing loss")
    ...
    parser.add_argument("--pclip_loss_weight", type=float, default=0.5, help="weight for P-CLIP lite random text perturbation + CCM loss")
    parser.add_argument("--pclip_noise_ratio", type=float, default=0.15, help="token replacement ratio for P-CLIP lite random text perturbation")
